-- Fix the catch_up_cow_depreciation_to_date function to respect the exact end date
-- The current function creates entries for the entire month, but we need to stop at the disposition date
...
BEGIN
  ...
  WHILE processing_date <= p_end_date LOOP
    -- For the last month, use the end date instead of end of month
    IF DATE_TRUNC('month', processing_date) = DATE_TRUNC('month', p_end_date) THEN
