-- Fix the journal_entry_id ambiguity by renaming the variable
-- The issue is that journal_entry_id is both a variable and a column name in journal_lines table

CREATE OR REPLACE FUNCTION public.process_monthly_depreciation_with_mode(
  p_company_id uuid, 
  p_target_month integer, 
  p_target_year integer,
  p_processing_mode text DEFAULT 'historical', -- 'historical' or 'production'
  p_current_month integer DEFAULT NULL,
...
DECLARE
  ...
  new_journal_entry_id UUID;  -- Renamed from journal_entry_id
  target_date DATE;
...
BEGIN
  ...
  SELECT je.id INTO new_journal_entry_id 
  FROM public.journal_entries je
  WHERE je.company_id = p_company_id 
    AND je.month = journal_month 
    AND je.year = journal_year 
    AND je.entry_type = 'depreciation'
    AND je.description LIKE '%' || TO_CHAR(target_date, 'Month YYYY') || '%';
  
  -- If entry exists, delete it and its lines to recreate
  IF new_journal_entry_id IS NOT NULL THEN
    DELETE FROM public.journal_lines WHERE journal_entry_id = new_journal_entry_id;
    DELETE FROM public.journal_entries WHERE id = new_journal_entry_id;
  END IF;
