  ...
  WHILE processing_date <= p_end_date LOOP
    ...
    IF journal_entry_id IS NOT NULL AND monthly_depreciation > 0 THEN
      ...
      IF NOT EXISTS (
        ...
        -- Update the journal entry total amount - FIXED: use variable name, not column
        UPDATE public.journal_entries 
        SET total_amount = (
          SELECT COALESCE(SUM(jl2.debit_amount), 0) 
          FROM public.journal_lines jl2
          WHERE jl2.journal_entry_id = journal_entry_id  -- Use variable, not table.column
        )
        WHERE id = journal_entry_id;  -- Use variable, not table.column
      END IF;
    END IF;
