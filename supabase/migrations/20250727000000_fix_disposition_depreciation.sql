-- Fix disposition depreciation calculation issues
-- This migration addresses partial month depreciation and ensures accurate book value calculations

-- Create enhanced function to calculate partial month depreciation
...
BEGIN
  ...
  -- Get disposition day and days in month
  disposition_day := EXTRACT(DAY FROM p_disposition_date);
  days_in_month := EXTRACT(DAY FROM (DATE_TRUNC('month', p_disposition_date) + INTERVAL '1 month - 1 day'));
  
  -- Calculate partial depreciation based on days elapsed in the month
  partial_depreciation := ROUND(monthly_depreciation * disposition_day / days_in_month, 2);
  
  RETURN partial_depreciation;
END;
$function$;

-- Create enhanced disposition processing function with proper partial month handling
