-- Fix depreciation entries created after cow disposition dates
-- This prevents and cleans up the issue where disposed cows continue to depreciate

-- First, identify and delete any depreciation journal entries created AFTER cow disposition dates
DELETE FROM journal_lines 
WHERE journal_entry_id IN (
  SELECT je.id 
  FROM journal_entries je
  JOIN journal_lines jl ON jl.journal_entry_id = je.id
  JOIN cows c ON c.id = jl.cow_id
...
-- Delete the corresponding journal entries
...
-- Update the monthly depreciation processing function to be more robust
...
BEGIN
  ...
  -- ENHANCED: More robust disposition checking
  FOR cow_record IN 
    ...
    WHERE c.company_id = p_company_id 
      ...
      AND c.status = 'active'  -- Only active cows
      -- Enhanced disposition check: cow must not be disposed OR disposed after target month
      AND (cd.disposition_date IS NULL OR cd.disposition_date >= (p_target_year || '-' || (p_target_month + 1) || '-01')::DATE)
  LOOP
    DECLARE
