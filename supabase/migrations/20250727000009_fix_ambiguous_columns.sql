-- Fix ambiguous column reference in the enhanced disposition function
-- The gain_loss column exists in both cow_dispositions and the calculated variable
...
BEGIN
  ...
  IF EXTRACT(DAY FROM disposition_record.disposition_date) < EXTRACT(DAY FROM disposition_month_end) THEN
    ...
    -- Check if we already have a depreciation entry for this month
    SELECT EXISTS (
    ...
    -- If we have an existing entry, we need to adjust it to partial month
  ...
  calculated_gain_loss := COALESCE(disposition_record.sale_amount, 0) - calculated_book_value;
  ...
  -- 4. Record gain/loss
  IF calculated_gain_loss != 0 THEN
    INSERT INTO public.journal_lines (
      journal_entry_id, account_code, account_name, description, debit_amount, credit_amount, line_type, cow_id
    ) VALUES (
      new_journal_entry_id, 
      CASE WHEN calculated_gain_loss > 0 THEN '4400' ELSE '4500' END, 
      CASE WHEN calculated_gain_loss > 0 THEN 'Gain on Sale of Assets' ELSE 'Loss on Sale of Assets' END, 
      CASE WHEN calculated_gain_loss > 0 THEN 'Gain on sale' ELSE 'Loss on sale' END || ' - Cow #' || disposition_record.tag_number, 
      CASE WHEN calculated_gain_loss > 0 THEN 0 ELSE ABS(calculated_gain_loss) END, 
      CASE WHEN calculated_gain_loss > 0 THEN calculated_gain_loss ELSE 0 END, 
      CASE WHEN calculated_gain_loss > 0 THEN 'credit' ELSE 'debit' END,
      disposition_record.cow_id
    );
  END IF;
