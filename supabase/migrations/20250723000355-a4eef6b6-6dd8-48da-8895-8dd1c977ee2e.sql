-- Fix the unique constraint to exclude both acquisition and disposition entries
-- Also fix the ambiguous column reference in the disposition processor

-- Drop the existing partial unique constraint
DROP INDEX IF EXISTS journal_entries_company_month_year_entry_type_unique;

-- Create a new partial unique constraint that excludes both acquisition and disposition entries
...
WHERE entry_type NOT IN ('acquisition', 'disposition');

-- Fix the process_missing_disposition_journals function to resolve ambiguous column reference
...
BEGIN
  ...
  -- Fix the ambiguous column reference by being explicit about which table the journal_entry_id belongs to
  FOR disposition_record IN 
    ...
    WHERE cd.company_id = p_company_id
      AND cd.journal_entry_id IS NULL  -- Explicitly reference cd.journal_entry_id
  LOOP
