DROP FUNCTION IF EXISTS public.catch_up_cow_depreciation_to_date(text,date);
...
BEGIN
  ...
  current_date := DATE_TRUNC('month', cow_record.freshen_date) + INTERVAL '1 month';
  ...
  WHILE current_date <= p_end_date LOOP
    ...
    IF journal_entry_id IS NOT NULL AND monthly_depreciation > 0 THEN
      ...
      IF NOT EXISTS (
        ...
        WHERE journal_entry_id = journal_entry_id AND cow_id = p_cow_id
      ) THEN
