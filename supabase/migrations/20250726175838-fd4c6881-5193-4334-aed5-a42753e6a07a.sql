-- Fix the ambiguous column reference in the disposition function
...
BEGIN
  ...
  calculated_gain_loss := ROUND(COALESCE(disposition_record.sale_amount, 0) - actual_book_value, 2);
  ...
  IF ABS(calculated_gain_loss) > 0.01 THEN
    DECLARE
      account_code TEXT;
      account_name TEXT;
    BEGIN
      ...
      INSERT INTO public.journal_lines (
        ...
        account_code, 
        account_name, 
        CASE WHEN calculated_gain_loss > 0 THEN 'Gain' ELSE 'Loss' END || ' on ' || disposition_record.disposition_type || ' - Cow #' || disposition_record.tag_number || ' (Book value: $' || actual_book_value || ')', 
        CASE WHEN calculated_gain_loss > 0 THEN 0 ELSE ABS(calculated_gain_loss) END, 
        CASE WHEN calculated_gain_loss > 0 THEN ABS(calculated_gain_loss) ELSE 0 END, 
