-- Create function to process acquisition journal entries
...
BEGIN
  ...
  -- Only create acquisition journal for purchased cows
  IF cow_record.acquisition_type != 'purchased' THEN
    RETURN jsonb_build_object('success', true, 'message', 'No journal entry needed for raised cows');
  END IF;
  ...
  -- Credit line for cash/accounts payable 
  INSERT INTO public.journal_lines (
    journal_entry_id, account_code, account_name, description, debit_amount, credit_amount, line_type, cow_id
  ) VALUES (
    journal_entry_id, 
