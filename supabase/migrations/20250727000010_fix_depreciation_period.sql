-- Fix the catch_up_cow_depreciation_to_date function to respect the depreciation period
-- The function should only create depreciation entries for the configured number of years (5 years)
-- not for the entire period from freshen date to disposition date
...
BEGIN
  ...
  WHILE processing_date <= effective_end_date LOOP
    -- For the last month, use the effective end date instead of end of month
    IF DATE_TRUNC('month', processing_date) = DATE_TRUNC('month', effective_end_date) THEN
      end_of_month := effective_end_date;
    ELSE
    ...
    IF current_journal_entry_id IS NULL AND monthly_depreciation > 0 THEN
      INSERT INTO public.journal_entries (
        ...
        end_of_month, -- Use end_of_month (which can be effective_end_date for the last month)
